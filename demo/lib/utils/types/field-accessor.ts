 ...
 */
export function createAccessibleField<T extends object>(obj: T): T & Accessible {
  if (isAccessible(obj)) {
    console.warn("Object already contains a '__path__' property. Ensure this is desired behavior, as `createAccessibleField` will overwrite this property with an empty string")
    if (typeof obj[ACCESSOR_PATH_NAME] !== "string") {
      throw new Error("Object's `__path__` property is not of type `string`.  Cannot overwrite")
    }
  }
  return {
    ...obj,
 ...
 */
export function makeStructureAccessible<T extends object, TargetField>(
  obj: T,
  guard: (value: unknown) => value is TargetField,
  parentPath: string = ''
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (guard(value)) {
      accessor[key] = createAccessibleField({ ...value as object, __path__: currentPath });
    } else if (typeof value === "object" && value !== null) {
      accessor[key] = makeStructureAccessible(value, guard, currentPath);
    }
  }
 ...
 */
export type Accessor<Base, TargetField> = Recursive<Base, AccessibleField<TargetField>>;
